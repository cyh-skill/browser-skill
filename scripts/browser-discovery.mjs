...
//   2. config.env 里 BROWSER_SKILL_BROWSER 设了 → 严格模式，找不到则硬错
...
export async function selectBrowser(override = null) {
  ...
  // 新变量名 BROWSER_SKILL_BROWSER，兼容 web-access 的旧变量名 WEB_ACCESS_BROWSER
  const cfg = readConfig();
  const configured = cfg.BROWSER_SKILL_BROWSER || cfg.WEB_ACCESS_BROWSER || null;
