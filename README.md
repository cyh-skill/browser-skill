...
| **双连接通道** | A: CDP-proxy（默认，全能力）；B: 扩展桥（免开调试开关，实验性） | 融合 |
...
```bash
node ~/.claude/skills/browser-skill/scripts/check-deps.mjs
```
...
| 通道 | 连接 | 何时用 |
|------|------|--------|
| **A · CDP-proxy**（默认） | `chrome://inspect` 开关 + Node 直连 | 日常首选，支持全部能力（含网络拦截）。已端到端验证 |
| **B · 扩展桥**（实验性） | 未打包扩展 `chrome.debugger` + Node WS 桥 | 免开调试开关、想要真·彩色标签组时。见 [`extension/README.md`](extension/README.md) |

两条通道 HTTP API 完全一致，任务里的调用与通道无关。详见 [`references/connection-channels.md`](references/connection-channels.md)。
